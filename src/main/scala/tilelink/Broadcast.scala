...
/** [[TLBroadcast]] parameters.
  *
  * @param lineBytes cacheline size.
  * @param numTrackers the number of outstanding transactions per bank, aka MSHR sizes.
  * @param bufferless if this is true. ready signal in A channel will directly decoupled from manager to client.
  * @param control add a control node which will be used for filter notification.
  */
...
/** [[TLBroadcast]] is the last level coherence manager of TileLink. Simplest topology can be diagrammed with this graph below:
  *
  *
  * {{{
  *   Client0     Client1
  ...
  * }}}
  *
  * - `Client0` and `Client1` are two [[TLClientNode]]s on the subsystem.
  * Basically they can be tiles, accerlators or some other nodes.
  *
  * - `BroadcastHub` is a [[TLAdapterNode]] which is the last level coherence device probing its clients' cachelines.
  * It can have or not have directory, depending on the implementation of [[ProbeFilter]].
  *
  * - `Manager` is a writeable memory device, which can be DDR, RAM.
  *
  * @example Here are two example transaction sequences on the broadcast.(`tx` is mark of an outstanding transaction)
  *          Scenario 1:
  *          if `Client0` s Tip, `Client1` acquires cacheline:
  *          (t0) `Client1` -> `BH`: AcquireBlock
  *          (t1) `BH` -> `Client0`: ProbeBlock
  *          (t1) `Client0` > `BH`: ProbeAckData/ProbeAck
  *          (t2) if ProbeAckData: `BH` -> `Manager`: PutFullData
  *          (t2)                  `Manager` -> BH: AccessAck
  *          (t3) `BH` -> `Manager`: Get
  *          (t3) `Manager` -> `BH`: AccessAckData
  *          (t0) `BH` -> `Manager`: GrantData
  *          (t0) `Manager` -> `BH`: GrantAck
  *
  *          if `Client0` is Tip, `Client0` eed to release TtoB:
  *          (t0) `Client0` -> `BH`: ReleaseData/Release TtoB
  *          (t1) `BH` -> `Manager`: Put (payload [[TLBroadcastConstants.TRANSFORM_B]] in the highest bits of source field.)
  *          (t1) `Manager` -> `BH`: AccessAck (check the highest bit to be 1 or 0,
  *          here is 1 since manager is replying to [[TLBroadcastConstants.TRANSFORM_B]] sourceId.))
  *          (t0) `BH` -> `Client0`:ReleaseAck
  *
  *          Behavior of [[TLBroadcast]]:
  *          1. receive A channel from client:
  *   - PutFullData    : send PutFullData to manager A channel with payload [[TLBroadcastConstants.PASS]] in highest 2 bits of source.
  *   - PutPartialData : send PutPartialData to manager A channel with payload [[TLBroadcastConstants.PASS]] in highest 2 bits of source.
  *   - ArithmeticLogic: send ArithmeticLogic to manager A channel with payload [[TLBroadcastConstants.PASS]] in highest 2 bits of source.
  *   - LogicalData    : send LogicalData to manager A channel with payload [[TLBroadcastConstants.PASS]] in highest 2 bits of source.
  *   - Get            : send Get to manager A channel with payload [[TLBroadcastConstants.PASS]] in highest 2 bits of source.
  *   - Intent         : send Intent to manager A channel with payload [[TLBroadcastConstants.PASS]] in highest 2 bits of source.
  *   - AcquireBlock   : send Get to manager A channel with payload in highest 2 bits of source.
  ...
  *   - AcquirePerm    : send Get to manager A channel with payload in highest 2 bits of source.
  *     payload depends on `a_param`:
  *     if `a_param` is `NtoB`, payload is [[TLBroadcastConstants.TRANSFORM_B]]
  *     if `a_param` is `NtoT` or `BtoT`, payload is [[TLBroadcastConstants.TRANSFORM_T]]
  *
  * 2. receive C channel from client:
  *   - ProbeAck       : notify [[TLBroadcastTracker.io.probenack]](Probe with no data is finished.)
  *   - ProbeAckData   : send PutFullData to manager A channel with payload [[TLBroadcastConstants.DROP]] in highest 2 bits of source.
  *   - Release        : send ReleaseAck to client D channel.
  *     notify [[ProbeFilter.io.release]]
  *   - ReleaseData    : send PutFullData to manager A channel with payload [[TLBroadcastConstants.TRANSFORM_B]] in highest 2 bits of source.
  *     notify [[ProbeFilter.io.release]]
  *
  * 3. receive E channel from client:
  *   - GrantAck       : accept only
  *     notify [[TLBroadcastTracker.io.e_last]]
  *
  * 4. receive D channel from manager:
  *   - if payload in highest 2 bits of source is [[TLBroadcastConstants.PASS]]:
  *     AccessAck      : send AccessAck to manager D channel
  *     AccessAckData  : send AccessAckData to manager D channel
  *     HintAck        : send HintAck to manager D channel
  ...
  *     AccessAck      : send ReleaseAck to manager D channel with `d_param` [[TLPermissions.toB]](replying to AcquirePerm)
  *     AccessAckData  : send GrantData to manager D channel with `d_param` [[TLPermissions.toB]](replying to AcquireBlock)
  *   - if payload in highest 2 bits of source is [[TLBroadcastConstants.TRANSFORM_T]]:
  *     AccessAck      : send ReleaseAck to manager D channel with `d_param` [[TLPermissions.toT]](replying to AcquirePerm)
  *     AccessAckData  : send GrantData to manager D channel with `d_param` [[TLPermissions.toT]](replying to AcquireBlock)
  *
  * 5. send to client in B channel:
  *   - Probe cachelines in `probe_todo` which is updated by [[ProbeFilter.io.response]].
  *
  * 6. send to client in D channel:
  *   - Convert D transaction from D channel of manager.
  * @note There are no line buffer in `BH`, so every ProbeAckData will consume more than 2x SoC to memory latency.
  */
class TLBroadcast(params: TLBroadcastParams)(implicit p: Parameters) extends LazyModule
{
    ...
    managerFn = { mp =>
      mp.v1copy(
        ...
        managers   = mp.managers.map { m =>
          // This is the last level coherence manager, so it cannot be probed by outward managers.
  ...
  lazy val module = new LazyModuleImp(this) {
    // For each bank, it has a pair of inward node and outward node. Bank number depends on the diplomacy negotiation.
    val (ints, fields) = node.in.zip(node.out).zipWithIndex.map { case (((in, edgeIn), (out, edgeOut)), bankIndex) =>
      ...
      // Since [[TLBroadcast]] doesn't implements PutPartialData. cacheline size must not be less than manager beatBytes.
      ...
      /** Use [[IdRange.start]] to locate which client is target. */
      ...
      /** create MSHR for outstanding transactions. */
      ...
      // -------- Client D Channel --------
      // Forward or transform manager D channel based on the payload of the source.

      /** Highest sourceId index. */
      val d_high = log2Ceil(edgeIn.client.endSourceId)
      /** payload extracted from source, used to store [[TLBroadcastConstants]]. */
      val d_what = out.d.bits.source(d_high+1, d_high)
      /** Indicate this transaction should be dropped:
        * send Probe client -> client reply ProbeAckData -> send Put(DROP) manager -> manager reply AccessAck(DROP)
        */
      val d_drop = d_what === DROP
      /** indicate this transaction has an available data field. */
      val d_hasData = edgeOut.hasData(out.d.bits)
      /** D channel to respond to clients. */
      val d_normal = Wire(in.d)
      /** [[d_first]] is the update enable signal of [[d_trackerOH]].
        * [[d_last]] tell MSHR and filter, it's the end of transaction in D schannel.
        */
      val (d_first, d_last, _) = edgeIn.firstlast(d_normal)
      /** select MSHR with:
        * 1. MSHR need d reply
        * 2. MSHR source is same with the client source.
        *
        * This is used for:
      ...
      /** [[d_allow]] is the signal allowing accept transaction from manager. */
      ...
      // size of source in client is less 2 bits than manager. truncates the higher 2 bits in source.
      ...
      // if is not TL-C transaction, just pass from manager to client.
      ...
      // Decrement the MSHR's outstanding probe counter.
      ...
      // To accept a request from A, the probe FSM must be idle and there must be a matching MSHR
      ...
      // Note: this assumes a cache doing MMIO has allocated
      filter.io.request.bits.allocOH := a_cache
...
/** [[ProbeFilter]] parameters.
  *
  * @param mshrs size of MSHR in [[TLBroadcast]]
  * @param caches size of clients caches.
  * @param maxAddress the largest legal physical address that will be filtered
  * @param addressMask set for those bits which can actually toggle in an address, ie:
  *                    - offset bits are 0
  *                    - bank bits are 0
...
/** a simple [[ProbeFilter]] without any metadata. */
...
class TLBroadcastTracker(id: Int, lineBytes: Int, caches: Int, bufferless: Boolean, edgeIn: TLEdgeIn, edgeOut: TLEdgeOut) extends Module
{
  val io = IO(new Bundle {
    ...
    /** output A channel to managers. */
    val out_a = Decoupled(new TLBundleA(edgeOut.bundle))
    /** input from filter for probing data. */
    ...
    /** wait for transaction in d channel. */
  ...
  /** state we got transaction in e channel.
    * client -> manager receive state has ended.
    */
  val got_e   = RegInit(true.B)
  /** state we sent transaction in d channel.
    * client -> manager send state has ended
    */
  ...
  /* clear some of the pending bits identifying which caches still need to be probed. */
